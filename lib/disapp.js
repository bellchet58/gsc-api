...
class DisApp {
  constructor({ getRequest }) {
    ...
    this.prefix = '/disapp'
    this.shortLinkPrefix = 'https://51gsc.com/app/'
  }
  ...
  async shortLink() {
    ...
    return `${this.shortLinkPrefix}${get('short_link')(data)}`
  }
  /**
   * 
  ...
  async action({ action, record_id, template = 2, icon, name, disapp_id, shortLink }) {
    if (action === 'create') {
      shortLink = await this.shortLink();
    }
    const setShortLink = action === 'create' ? set('shortLink', shortLink): identity;
