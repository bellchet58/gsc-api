...
class OSS {
  ...
  getOSSFormData({ host, dir, policy, signature, callback, accessid, filePath = 'Expo_v2.9.0.apk' }) {
    ...
    const inputPath = `${process.env.UPLOAD_DIR ? path.join(process.env.UPLOAD_DIR, filePath) : filePath}`
    filePath = !path.isAbsolute(filePath) ? path.resolve(basePath, `./${inputPath}`): filePath;
    const params = {
      host,
